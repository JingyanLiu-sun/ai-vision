      ...
      try {
        ...
        if (route.includes("[chartId]")) {
          ...
          for (const config of dynamicChartConfigs) {
            ...
            const chartraceData = dict.seo?.chartrace?.[config.id] || {};
            const metadata = {
              title: chartraceData.title || config.title,
              description: chartraceData.description || config.description,
          ...
          for (const template of documentTemplates) {
            ...
            const gendocxTempData = dict.gendocx_temp || {};
            const metadata = {
              title: dict[template.id] || template.id,
              description: gendocxTempData[template.id] || template.id,
              canonicalUrl: `${DOMAIN}/${lang}${dynamicRoute}`,
