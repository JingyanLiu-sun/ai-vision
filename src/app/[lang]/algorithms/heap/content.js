...
const HeapVisualization = () => {
  ...
  // 从 localStorage 加载保存的进度（用于页面刷新时显示提示）
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem('heapVisualizationProgress', JSON.stringify(progress));
      setHasSavedProgress(true);
    } catch (error) {
  ...
  const handleClearProgress = useCallback(() => {
    try {
      localStorage.removeItem('heapVisualizationProgress');
      setHasSavedProgress(false);
      
      // 重置为默认堆
      const newHeap = new Heap(HEAP_TYPES.MAX);
      [50, 30, 20, 15, 10, 8, 16, 4, 5, 6, 100, 300].forEach((value) => newHeap.insert(value));
      setHeap(newHeap);
      setHeapType(HEAP_TYPES.MAX);
    } catch (error) {
