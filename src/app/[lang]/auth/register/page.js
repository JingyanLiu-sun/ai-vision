  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white/90 backdrop-blur rounded-2xl shadow-lg p-6 sm:p-8 border border-gray-100">
        <h1 className="text-2xl font-bold mb-3 text-gray-900">{t("register_title")}</h1>
        <p className="text-sm text-gray-600 mb-6">{t("register_subtitle")}</p>
        {error && <p className="text-red-600 text-sm mb-3">{error}</p>}
        {ok && <p className="text-green-600 text-sm mb-3">{t("register_success")}</p>}
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-gray-900 text-white hover:bg-gray-800 transition shadow-sm">{t("register_submit")}</button>
        </form>
        <div className="mt-4 text-center text-sm text-gray-600">
          <a href={`/${lang}/auth/signin`} className="text-blue-600">{t("go_signin")}</a>
        </div>
      </div>
    </div>
  );
